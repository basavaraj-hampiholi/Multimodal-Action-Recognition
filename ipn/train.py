...
def train_epoch(epoch, data_loader, model, criterion, optimizer, opt,
    ...
    for i, (rgb_inputs, seg_inputs, targets) in enumerate(data_loader):
        data_time.update(time.time() - end_time)

        #if not opt.no_cuda:
        #    targets = targets.cuda(async=True)
        rgb_inputs = Variable(rgb_inputs.float().cuda())
        seg_inputs = Variable(seg_inputs.float().cuda())
        targets = Variable(targets.long().cuda())
        #pdb.set_trace()
        outputs = model(rgb_inputs, seg_inputs)
        ...
        precisions.update(precision, rgb_inputs.size(0))
        recalls.update(recall,rgb_inputs.size(0))
