    ...
    if modality in ['RGB', 'flo', 'seg']:
        for i in frame_indices:
            ...
            if os.path.exists(image_path):
                ...
                rgb_video.append(image_loader(image_path, modality))
            else:
        ...
        # seg1CM42_21_R_#156_000076
            ...
            if os.path.exists(image_path):
                rgb_video.append(image)
                depth_video.append(image_depth)
            else:
                print(image_path, "------- Does not exist")
...
class IPN(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        rgb_clip, depth_clip = self.loader(path, frame_indices, self.modality, self.sample_duration)
        ...
        im_dim_r = rgb_clip[0].size()[-2:]
        im_dim_d = depth_clip[0].size()[-2:]
        rgb_clip = torch.cat(rgb_clip, 0).view((self.sample_duration, -1) + im_dim_r).permute(1, 0, 2, 3)
        depth_clip = torch.cat(depth_clip, 0).view((self.sample_duration, -1) + im_dim_d).permute(1, 0, 2, 3)
        
        target = self.data[index]
        if self.target_transform is not None:
            target = self.target_transform(target)

        seg_clip = torch.cat((depth_clip,depth_clip),dim=0)

        return rgb_clip, seg_clip, target
