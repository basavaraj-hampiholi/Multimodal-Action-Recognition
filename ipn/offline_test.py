...
model.load_state_dict(torch.load(opt.resume_path)["state_dict"])
...
inst_ids.append(-1)
out_queue = []
for i, (inputs_rgb, inputs_flo, targets) in enumerate(test_loader):
    if not opt.no_cuda:
        targets = targets.cuda(async=True)
        inputs_rgb = inputs_rgb.cuda()
        inputs_flo = inputs_flo.cuda()
    #inputs = Variable(torch.squeeze(inputs), volatile=True)
    end_time = time.time()
    with torch.no_grad():
        inputs_rgb = Variable(inputs_rgb)
        inputs_flo = Variable(inputs_flo)
        targets = Variable(targets)
        outputs = model(inputs_rgb, inputs_flo)
        softmax = nn.Softmax()
    ...
    if outputs.size(1) <= 4:
        ...
        if not opt.true_valid:
            ...
            top1.update(prec1[0], inputs_rgb.size(0))
            precisions.update(precision, inputs_rgb.size(0))
            recalls.update(recall,inputs_rgb.size(0))
        ...
        if not opt.true_valid:
            ...
            top1.update(prec1, inputs_rgb.size(0))
            top5.update(prec5, inputs_rgb.size(0))
            precisions.update(precision, inputs_rgb.size(0))
            recalls.update(recall,inputs_rgb.size(0))
