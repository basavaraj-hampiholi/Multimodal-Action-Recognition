...
class DepthwiseConv(nn.Module):
    ...
    Conv layers in efficient self-attention are replaced with 3D DepthwiseConv layers to learn spatio-temporal features.
    This block introduces very few additional parameters to the two-stream network.
...
class EfficientAttention(nn.Module):   
    ...
    Modified Efficient Attention: This version can process spatio-temporal data using 3D depthwise separable convolutions.
...
class FeedForwardNet(nn.Module):
    ...
    A module with two fully connected layers to compute global features of the fused features.
