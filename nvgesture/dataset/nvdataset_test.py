...
class NVGestureDataset(Dataset):
    ...
    def __init__(self, root_dir, lst_file_input, seg_len, train, transform=None):
        ...
        self.video_crop = Aug.RandomCrop(224)
        self.train = train
    ...
    def __getitem__(self, idx):
        ...
        rgb_list = 0#self.make_dataset(rgb_path)
        depth_list = 0#self.make_dataset(depth_path)

        #rgb_list = [i for i in range(16,self.seg_len+16)]
        #depth_list = [i for i in range(16,self.seg_len+16)]

        rgb_vid = self.read_video(rgb_path, rgb_list)#+(0.1**0.5)*np.random.rand(64, 240, 320, 3)    

        depth_vid = self.read_video(depth_path, depth_list)
...
def training_dataset(root_dir, lst_file_input, seg_len, train):
    video_rotate_augment = [video_transforms.Resize(256),
                            ...
                            video_transforms.RandomHorizontalFlip(1.0),
