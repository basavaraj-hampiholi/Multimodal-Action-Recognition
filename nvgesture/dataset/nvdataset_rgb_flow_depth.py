...
class NVGestureDataset(Dataset):
    ...
    def read_flow(self, path, idx_list):
        vid=vio.vread(path+'/sk_flow.mp4')
        flows = []
        for i in idx_list:
            frame = vid[i,:,:,:]
            flows.append(frame)
    ...
    def __getitem__(self, idx):
        ...
        flow_vid = self.read_flow(rgb_path, rgb_list)     
        ...
        return rgb_video, flow_vid[0:2,:,:,:], depth_video, label
...
def training_dataset(root_dir, lst_file_input, seg_len, train):
    ...
    #               transform=video_transforms.Compose([video_transforms.RandomVerticalFlip(1.0),volume_transforms.ClipToTensor()]))
