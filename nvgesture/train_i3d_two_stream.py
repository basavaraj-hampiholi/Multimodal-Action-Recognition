...
from models.early import EarlyFusion
...
# def load_weights(path, type):
#     if type=='rgb':
#         res50 = generate_model(model_depth=50, n_classes=700)
#         #res50.load_state_dict(torch.load('load_model/r3d50_K_200ep.pth')['state_dict'])
#         res50.fc = nn.Linear(2048, num_classes)
#         #res50.load_state_dict(torch.load(path))
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input_rgb, input_depth, target) in enumerate(train_loader):
        ...
        cos_losses.update(sim_loss.data, input_rgb.size(0))
        ...
        if i % print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Sim Loss {cos_loss.val:.4f} ({cos_loss.avg:.4f})\t'
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                   epoch, i, len(train_loader), batch_time=batch_time,
                   data_time=data_time, loss=losses, cos_loss=cos_losses, top1=top1, top5=top5))
      ...
      for i, (input_rgb, input_depth, target) in enumerate(val_loader):
          ...
          if i % print_freq == 0:
              print('Test: [{0}/{1}]\t'
                    ...
                    'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                    'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                     i, len(val_loader), batch_time=batch_time, loss=losses, cos_loss=cos_losses,
                     top1=top1, top5=top5))
