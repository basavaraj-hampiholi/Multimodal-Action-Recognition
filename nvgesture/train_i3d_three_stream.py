...
from models.fusion_model3m import TransFusion
from dataset.nvdataset_rfd import training_dataset, test_dataset
...
def main():
    ...
    i3d_flow = load_weights('load_model/nv_flow_best.pt', 'flow')
    i3d_depth = load_weights('load_model/nv_depth_best.pt', 'rgb')
    ...
    model = TransFusion(copy.deepcopy(i3d_rgb), copy.deepcopy(i3d_flow), copy.deepcopy(i3d_depth), num_classes)   
    model = model.cuda()
    #model.load_state_dict(torch.load('load_model/nv_fusion_best.pt'))

    #summary(model, [(3,16,224,224), (3,16,224,224)])
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input_rgb, input_flow, input_depth, target) in enumerate(train_loader):
        ...
        # compute output
        output = model(input_rgb, input_flow, input_depth)
...
def validate(val_loader, model, criterion, epoch):
    ...
    with torch.no_grad():
      for i, (input_rgb, input_flow, input_depth, target) in enumerate(val_loader):
          ...
          if use_gpu:
             ...
             input_rgb = torch.autograd.Variable(input_rgb.float())
             input_flow = torch.autograd.Variable(input_flow.cuda())
             input_depth = torch.autograd.Variable(input_depth.float())
             target = torch.autograd.Variable(target.long())
