...
class TransFusion(nn.Module):
    def __init__(self, i3d_rgb, i3d_flow, class_nb=101):
        ...
        self.att_in = ResNet_Attn(self.inchannels1, n_heads=3)
    ...
    def forward(self, x_rgb, x_flow):
        ...
        att_in = self.conv1(self.att_in(rgb_in, flow_in))

        rgb1_fuse = rgb_in*(1+att_in)
        flow1_fuse = flow_in*(1+att_in)
        rgb1 = self.rgb_l1(rgb1_fuse)
        flow1 = self.flow_l1(flow1_fuse)

        att1 = self.conv2(self.att1(rgb1, flow1))

        rgb2_fuse = rgb1*(1+att1)
        flow2_fuse = flow1*(1+att1)
        rgb2 = self.rgb_l2(rgb2_fuse)
        flow2 = self.flow_l2(flow2_fuse)

        att2 = self.conv3(self.att2(rgb2, flow2))
        
        rgb3_fuse = rgb2*(1+att2)
        flow3_fuse = flow2*(1+att2)
        rgb3 = self.rgb_l3(rgb3_fuse)
        flow3 = self.flow_l3(flow3_fuse)
      
        att3 = self.conv4(self.att3(rgb3, flow3))
